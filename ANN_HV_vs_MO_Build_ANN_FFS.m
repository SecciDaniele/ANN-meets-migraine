...
% Train final model with selected features by FFS
X_selected = X(:, [5 6 10]);
...
for j = 1:num_trials
    ...
    % net.performFcn = 'mse'; % Change performance function
    
    % Train the Network
    [net, tr] = train(net, X_selected', dummyvar(Y_categorical)');
    % [net, tr] = train(net, X_selected',(double(Y_categorical) - 1)'); % Convert to binary labels (0 or 1)
